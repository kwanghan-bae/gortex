...
- **Verification**: `PYTHONPATH=/Users/joel/Desktop/git coverage run -m pytest` → `coverage report`로 전체 78% 커버리지를 달성하고 다음 타깃(`utils/notifier.py`, `core/graph.py`)을 설정했습니다.
