...
### 5.1 Strict TDD Cycle (테스트 주도 개발)
모든 코딩 작업은 다음 사이클을 엄격히 준수한다.
1.  **Test Design**: 구현하려는 기능의 테스트 케이스를 먼저 생각하고 `tests/`에 작성한다.
2.  **Implementation**: 테스트를 통과하기 위한 코드를 작성한다.
3.  **Verification**: `python -m unittest ...`를 실행하여 초록불(Pass)을 확인한다.
*   **경고**: 테스트 없이 코드만 작성하다가 적발되면, 해당 코드는 "신뢰할 수 없는 환각"으로 간주되어 삭제될 수 있다.
