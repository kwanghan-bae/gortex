...
## 📊 강력한 터미널 관측성 (Rich TUI Dashboard)
Gortex는 블랙박스가 아닙니다. **Rich 라이브러리 기반의 고성능 TUI**를 통해 에이전트의 사고 과정(**Thought Tree**), 아키텍처 건강도(**Health Score**), 그리고 진화 이력을 터미널 안에서 실시간으로 투명하게 공개합니다. 웹 브라우저 없이 터미널 하나로 모든 제어와 관측이 가능합니다.
...
Gortex 터미널에서 다음 명령어를 통해 시스템을 직접 제어할 수 있습니다.

| 명령어 | 설명 |
| :--- | :--- |
| `/help` | 모든 명령어 및 사용법 안내 출력 |
| `/status` | 현재 토론 횟수, 토큰 사용량, 가상 에너지 상태 보고 |
| `/search [검색어]` | 프로젝트 소스 코드 내 의미 기반(Semantic) 심볼 검색 |
| `/map` | 전체 프로젝트 구조(파일/클래스/함수)를 트리 형태로 출력 |
| `/scan_debt` | 기술 부채 및 코드 복잡도 정밀 분석 수행 |
| `/mode [mode]` | UI 모드 변경 (`coding`, `research`, `debugging`, `analyst`, `standard`) |
| `/language [ko\|en]` | 시스템 응답 및 UI 언어 즉시 변경 |
| `/theme [theme]` | TUI 대시보드 테마 변경 |
| `/rca [id]` | 특정 이벤트의 근거가 된 인과 관계(Root Cause) 역추적 |
| `/clear` | 터미널 대화 내역 초기화 |

### 2. 자가 진화 피드백 루프 (Evolution Feedback)
Gortex는 당신의 피드백을 통해 성장합니다.
*   **제약 조건 학습**: 에이전트의 결과물이 마음에 들지 않을 때 구체적인 지침(예: "항상 snake_case를 써줘")을 주면, `Analyst`가 이를 `experience.json`에 영구 규칙으로 각인합니다.
*   **자동 리팩토링**: `/scan_debt`를 통해 발견된 취약점은 `EvolutionNode`가 에너지가 충분할 때 스스로 코드를 수정하여 치유합니다.

### 3. 하이브리드 모델 설정 (Hybrid Setup)
Gortex는 클라우드(Gemini)와 로컬(Ollama) 모델을 결합하여 최적의 가성비를 도출합니다.
*   **Gemini**: 고수준 설계, 복잡한 의도 분석, 아키텍처 결정 시 사용.
*   **Ollama**: 단순 코드 구현, 단위 테스트 생성, 반복적인 요약 작업 시 사용.
*   **설정**: `.env` 파일에서 각 에이전트별 기본 모델을 지정할 수 있으며, 시스템이 성과 데이터에 따라 실시간으로 모델을 스위칭합니다.
